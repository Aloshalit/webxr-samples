...
var generateInnerHTML = function generateInnerHTML(cssPrefix, height) {
  var logoHeight = 0; //height*_LOGO_SCALE;
  var svgString = generateXRIconString(cssPrefix, logoHeight) + generateNoXRIconString(cssPrefix, logoHeight);
...
var createXRIcon = function createXRIcon(cssPrefix, height) {
  var el = document.createElement('div');
  el.innerHTML = generateXRIconString(cssPrefix, height);
  return el.firstChild;
};

var createNoXRIcon = function createNoXRIcon(cssPrefix, height) {
  var el = document.createElement('div');
  el.innerHTML = generateNoXRIconString(cssPrefix, height);
  return el.firstChild;
};

var generateXRIconString = function generateXRIconString(cssPrefix, height) {
  var aspect = 28 / 18;
...
var generateNoXRIconString = function generateNoXRIconString(cssPrefix, height) {
  var aspect = 28 / 18;
...
export var WebXRButton = function () {
  /**
   ...
   */
  function WebXRButton(options) {
    var _this = this;

    _classCallCheck(this, WebXRButton);
    ...
    this.domElement.addEventListener('click', function () {
      return _this.__onXRButtonClick();
    });
  ...
  _createClass(WebXRButton, [{
    ...
    key: '__onXRButtonClick',
    value: function __onXRButtonClick() {
      var _this2 = this;

      if (this.session) {
        this.options.onEndSession(this.session);
      } else if (this._enabled) {
        var requestPromise = this.options.onRequestSession();
        if (requestPromise) {
          requestPromise.catch(function (err) {
            ...
            var errorMsg = 'XRSession creation failed: ' + err.message;
            _this2.setTooltip(errorMsg);
            console.error(errorMsg);

            // Disable the button momentarily to indicate there was an issue.
            _this2.__setDisabledAttribute(true);
            _this2.domElement.setAttribute('error', 'true');
            setTimeout(function () {
              _this2.__setDisabledAttribute(false);
              _this2.domElement.setAttribute('error', 'false');
            }, 1000);
          });
        }
      }
    ...
    key: '__updateButtonState',
    value: function __updateButtonState() {
      if (this.session) {
